...
export const mostrarLocatarios = () => async dispatch => {
    const respuesta = await axios.get("http://192.168.1.102:5000/getlocatarios");
    dispatch({
...
export const mostrarLocatario = id => async dispatch => {
    const respuesta = await axios.get(`http://192.168.1.102:5000/getlocatarioselect/${id}`);
...
export const agregarLocatarioContrato = locatario => async dispatch => {
    const respuesta = await axios.post(
        "http://192.168.1.102:5000/postlocatariocontrato",
        locatario
    );
